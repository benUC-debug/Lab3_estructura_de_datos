...
HashMap * createMap(long capacity) {
    ...
    nuevoMap->size = 0;
    nuevoMap->current = -1;
    nuevoMap->buckets = (Pair **) malloc(sizeof(Pair *)* capacity);
    if(nuevoMap->buckets == NULL){
        return NULL;
    }    
    for(long i = 0; i < capacity; i++){
        nuevoMap->buckets[i] = NULL;
    }
    return nuevoMap;
}
...
void insertMap(HashMap * map, char * key, void * value) {
    ...
    while(map->buckets[pos] != NULL){
        if(map->buckets[pos]->key != NULL && strcmp(map->buckets[pos]->key,key) == 0) return;
        if(map->buckets[pos]->key == NULL) break;
        pos = (pos + 1) % map->capacity;
        if (pos == inicial) return;
    }
...
Pair * searchMap(HashMap * map,  char * key) {  
    ...
    while(map -> buckets[pos] != NULL){
        if(strcmp(map -> buckets[pos]->key, key)== 0){//lista de los pares (key,data)
            map->current = pos;
